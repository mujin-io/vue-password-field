import {defineComponent, reactive, createVNode, watch} from 'vue';
...
export default defineComponent({
	...
	setup({
		...
		const messagesView = () => {
			let viewsList = requirementRules.filter(v => requirements.some((k:string) => k === v.key)).map(r => {
				let isPassed = r.key !== 'notContainAccounts' ? r.rule(data.password)
					: r.rule(data.password, (p: string) => {
						return p.length > 0 && ! containAccounts.filter(v => v !== undefined || v !== '' || v !== null)
							.map(v => v.toLowerCase())
							.every(v => p.includes(v));
					});
				return createVNode('li', {
					class: 'flex items-center ' + (isPassed ? 'text-green-600' : 'text-red-600'),
					'data-id': 'PasswordStrength-' + r.key
				}, [
					createVNode('span', {class: 'w-4 h-4 mr-2'}, [
						createVNode('svg', {
							viewBox: '0 0 48 48',
							class: 'fill-current',
						}, [
			...
			viewsList.push(createVNode('li', {class: 'flex items-center text-sm text-gray-700', 'data-id': 'PasswordStrength-notContainAccounts'}, [
				createVNode('span', {class: 'flex h-2 w-2 relative mr-2'}, [
				...
				createVNode('span', {}, trans('is not commonly used')),
			]));
			...
			createVNode('div',
				{
					...
					createVNode('button', {
						class: 'm-0 ml-3 absolute inline-flex inset-y-0 right-0 flex items-center pr-2 cursor-pointer no-underline visible',
						type: 'button',
